...
public class ExtraInfoCrawler extends BaseService {
	...
	public void requestReadme(String repoName, Document doc) {

		Gson gson = new Gson();
		GithubUrl readmeUrl = new GithubUrl("https://api.github.com/repos/" + repoName + "/contents/README.md");
		HttpRequest readmeReq = null;
		try {
			readmeReq = requestFactory.buildGetRequest(readmeUrl);
	...
	public Document webProcess(String repoName) throws IOException {
		Document doc = new Document("full_name", repoName);

		GithubUrl fullInfoUrl = new GithubUrl("https://api.github.com/repos/" + repoName);
		HttpRequest fullInfoReq = requestFactory.buildGetRequest(fullInfoUrl);
		String response = fullInfoReq.execute().parseAsString();
		doc = Document.parse(response);
